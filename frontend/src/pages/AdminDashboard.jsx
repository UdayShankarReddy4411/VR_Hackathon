...
export default function AdminDashboard() {
  ...
  return (
    <div className="max-w-6xl mx-auto animate-in fade-in duration-500">
      <div className="mb-8">
        <h1 className="text-3xl font-extrabold text-indigo-900 mb-2">Platform Administration</h1>
        <p className="text-gray-500">Manage NGOs, moderate content, and oversee the platform ledger.</p>
      </div>

      <div className="flex gap-4 border-b border-gray-200 mb-6">
        <button onClick={() => setActiveTab('users')} className={`pb-3 px-2 font-bold text-sm ${activeTab === 'users' ? 'border-b-2 border-indigo-600 text-indigo-600' : 'text-gray-500 hover:text-gray-700'}`}>
          <Users className="inline h-4 w-4 mr-2" /> NGOs
        </button>
        <button onClick={() => setActiveTab('causes')} className={`pb-3 px-2 font-bold text-sm ${activeTab === 'causes' ? 'border-b-2 border-indigo-600 text-indigo-600' : 'text-gray-500 hover:text-gray-700'}`}>
          <AlertTriangle className="inline h-4 w-4 mr-2" /> Causes & Moderation
        </button>
        <button onClick={() => setActiveTab('donations')} className={`pb-3 px-2 font-bold text-sm ${activeTab === 'donations' ? 'border-b-2 border-indigo-600 text-indigo-600' : 'text-gray-500 hover:text-gray-700'}`}>
      ...
      {activeTab === 'donations' && (
        <div className="space-y-4">
          <div className="flex justify-end">
            <button onClick={exportLedger} className="flex items-center gap-2 bg-indigo-600 text-white px-4 py-2 rounded-xl font-bold hover:bg-indigo-700 shadow-sm transition-colors">
              ...
              <tbody>
                {donations.map(d => (
                  <tr key={d._id} className="bg-white border-b border-gray-50">
                    ...
                    <td className="px-6 py-4 font-bold text-indigo-600">${d.amount}</td>
